...
class CheckoutController extends Controller
{
    ...
    public function Checkout()
    {
        ...
        return view('frontend.cart.checkout',compact('content','shipping'));
          ...
          if ($check) {
              ...
              if (date('Y-m-d', strtotime(date('Y-m-d'))) <= date('Y-m-d', strtotime($check->valid_date))) {
                   session::put('coupon',[
                      ...
                      'after_discount'=>$check->coupon_amount
                   ]);
    ...
    public function OrderPlace(Request $request)
    {
        ...
        if($request->payment_typp == "Hand cash"){
            $order=array();
           ...
           // dd($order);
            $order_id=DB::table('orders')->insert($order);
            ...
            $notification=array('message' => 'Successfullt Order Placed!', 'alert-type' => 'success');
