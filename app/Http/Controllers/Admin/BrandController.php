...
class BrandController extends Controller
{
    ...
    public function index(Request $request){
        if($request->ajax()){
            ...
            return DataTables::of($data)->addIndexColumn()->editColumn('front_page',function($row){
                if($row->front_page == 1){
    ...
    public function store(Request $request){
        ...
        $data=array();
         ...
         $data['front_page']=$request->front_page;
          ...
          $image =  Image::make($photo)->resize(240,120)->save('public/files/brand/'.$photoname);  //image intervention
